...
//  JustDoIt
//
//  Created by Kirill Tarasko on 14.02.2023.
//
...
class TaskListViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "editTask" {
            guard let newTaskVC = segue.destination
                    as? NewTaskViewController else {
                return }
    ...
    private func setContentForCell(with task: Task?) -> UIListContentConfiguration {
        ...
        content.textProperties.font = UIFont(name: "Avenir Next Medium", size: 23) ?? UIFont.systemFont(ofSize: 23)
    ...
    private func strikeThrough(string: String, _
                               isStrikeThrough: Bool) -> NSAttributedString
    {
        var attributedString = NSAttributedString(string: string,
